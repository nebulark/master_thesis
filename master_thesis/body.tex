...
\section{Implementation 11.5/30}
...
\subsection{Dynamic Portal Rendering 1}
...
\subsubsection{Visible Portal Number algorithm 0.75}
...
Thus if at least one fragment shader was invoke for the portal, there will be a non zero value at its position. As the element which is written to, is not read, the write does not influence the other shader invocations.


 \textbf{ \huge TODO: Undefined behaviour? }%TODO: check for data race

The written value may can be any non-zero value, but writting the previous visible portal count allowed for easier debugging during the creation of the implementation.





\subsubsection{Camera Indices Index}
...
\subsubsection{Occlusion query as alternative}
probably slow
