...
\subsubsection{Transformative Portals}
...
\subsubsection{Portal Recursion}
...
\subsection{Reasons For Portals}
Portals are applicable for many different topics. Portals can be used to accelerate rendering speed when used in \glspl{pvs} \cite{luebke:1995:portals} algorithms. Although they are not quite portals, magic lenses are very similar to portals and allow for useful visualisations \cite{viega:1996:3d}. For video games  portals have multiple uses. Not do they improve performance using \gls{pvs} acceleration techniques, but also serve as interesting gameplay mechanics. Furthermore they allow for intuitive transitions between worlds \cite{schmalstieg:1999:sewing} or intuitive locomotion for \gls{vr} games. 
...
\subsubsection{Early Fragment Test}
...
OpenGL stands for Open Graphics Library and is a cross platform \gls{api} for graphics hardware. It is and open standard of the Khronos Group. OpenGL defines a set of commands available to a programmer. This set of commands are implemented by driver implementors for specific \gls{gpu}. The programmer does not need to know which \gls{gpu} will be used in the program. Some operations on the \gls{gpu} are fixed and can only have their settings configured. Other operations are fully programmable using shaders \cite{khronos:glsl4.60:spec}.

OpenGL uses \Gls{glsl} as its programming Language. It is a human readable programming language for defining shaders, which is is very similar to the C programming language. \Gls{glsl} is a actually set of very close languages, for each programmable part of the graphics pipeline. Some keywords are only available in a subset of those languages. There is a small amount of built in variables that influence the pipeline or made available for use in the shader \cite{khronos:glsl4.60:spec}.


\subsection{Vulkan}
Vulkan is very similar to OpenGL and is also specified by the Khronos Group. It is designed for more explicit control of low-level graphics and compute functionality \cite{khronos:vulkan:spec1.1}. The following sections give a quick overview of important aspects of Vulkan that are needed for the implementation.
...
Whenever different settings are needed for drawing objects, an additional pipeline needs to be created. For example, if there are two objects which need different shaders, each of the objects needs its own pipeline to draw it. However, there are a few values that can be configured to be dynamically using dynamic state. Static settings for specified dynamic state are ignored during pipeline creation and must be set dynamically before rendering \cite{khronos:vulkan:spec1.1}.
...
\subsection{Transformative Portals in Games}
...
One important aspect of the prototpye was that it can run on as many plattforms as possible. %TODO Reasoning: Should / Where Should it state the reason for supporting more plattforms
...
\subsubsection{Transformative Portal}
...
As the implementation has a depth range of 0 to 1, the value will always be positive. This allows the implementation to store the winding order as the sign of the near buffer. Positive values indicating front facing and negative values indicating back facing. Near Depth comparison is done with the absolute value of the buffer.
...
\subsubsection{Demo Scene Render Baseline}
...
\section{Performance Improvement Opportunities}
...
\subsubsection{Excluding the initial view matrix}
...
\subsubsection{Calculating on the GPU}
...
\subsubsection{Portal Shadow Mapping}
...
\subsubsection{Portals as Light Source}
...
\subsubsection{Halfway through Objects}
...
\subsection{Form changing portals ?}
