...
\subsection*{Search Terms TOTAL PAGES 46.95/69}
portal
3D lense
magic lense
Seams
...
\section{Graphics programming 2.7/9}
...
\subsection{Portals use in Sewing worlds together with SEAMs 0.5}
...
\subsection{Magic Lense}
Another topic that uses portal rendering techniques are magic lenses. A magic lense is a region, which has a special operation applied. For instance these operation could be magnification, rendering in wireframe, changing color, etc. . Magic lenses are a powerful tool for visualization  \cite{bier:1993:toolglass, tominski:2014:survey}.
...
\section{Analysis And Improvements 13/15}
...
Each Portal's bounding box is projected into screen space. However, instead of using a perspective matrix, only the X and Y coordinates are just divided by Z. This leaves Z instact which is useful for further calculations. Around the projected points a \gls{aabb} is created, which will be called \gls{psb}.  However, for non watertight portals the max Z value must be set to infinite.The \glspl{psb} are calculated recursion by recursion. For recursion 1 and onward, the actual bounding box is the intersection with the  \gls{psb} of its direct \gls{parentportal}. For recursion 0 its an intersection with a box with x and y of -1 and 1 for min and max respectively. Min z is 0 and max z is infinite. Only portals are consider in this approach, objects are ignored. Portals with an \gls{psb} without volume are not visible.
...
One way to achieve this is by decreasing the clip volume. Regularly the view frustum is used as clip volume for primitive clipping. However, as the portals is only at part of the screen primitives should be clipped to this much smaller volume. This volume is referred to as \gls{portalfrustrum}. The \glspl{psb} introduced in section \ref{section:cullingportals} can be used to calculated the frustum. The \gls{psb} must be available in the vertex shader. The individual clip distances can be calculated via subtractions. Note that while Z can be directly used, the  x and y coordinate must be in the same space as the ones of \gls{psb}. Multiplying the x and y of the \gls{psb} seems to be preferable to diving the vertex's x an y by its z. The only difference between those two possibilities is that the clip distances are scaled by z. This should not matter for the primitive clipping \cite{khronos:vulkan:spec1.1}.

The regular clip planes for the view frustum can then also be disabled, as the \gls{portalfrustrum} is a subspace of it. The vertex shaders must be extended to calculated the distances to the \gls{portalfrustrum}['s] clip planes.


\subsection{CPU Portal Frustum Culling 0.3}
The results from section \ref{section:cullingportals} can be reused to perform a variant of frustum culling. The projected objects can be tested against the \glspl{psb}. One way to do this is by using the bounding spheres of the objects. The center point is first multiplied with the corresponding view matrix. Then a axis aligned cube is created by using the radius of the bounding sphere and tested against the corresponding portal box. The \gls{psb}['s] x and y coordinate are multiplied by the min z of the bounding cube before comparision, similar to described in section \ref{section:portalprimitiveclipping}.


If the axis aligned cube lies outside of the \gls{psb} the object can be culled. A different bounding box and approach can be used, but it should be fast to test, while not being too conservative.
