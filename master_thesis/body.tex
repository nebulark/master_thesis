...
\subsection*{Search Terms TOTAL PAGES 36.75/69}
portal
3D lense
magic lense
Seams
...
\section{Graphics programming 0/9}
...
\section{Previous Work 0/7}

\subsection{Transitioning between worlds}

\textcite{schmalstieg:1999:sewing} used portals to connect different virtual worlds together.

\subsection{Portals as Occlusion Culling}

similarities between portals and transformative portals.
...
\section{Analysis And Improvements 11.95/15}
...
\subsection{Shadows}
Due to time reasons the implementation has no shadows implemented. This section possible approach that would have been used, if time had allowed. For a portal to appear seamless care must be taken when rendering shadows. Portals must be taken into account, or otherwise shadows appear cut of or appear out of nowhere in the presence of a portal.




\subsubsection{Rethink again? Portal Shadow Mapping}
...
Then when rendering the actual scene, the shadow test is performed. However, as it is not know whether the light travelled through a portal the position on the shadow map can not be calculated directly. The following needs to be done for each matrix and matrix combination the light could have used: 
Apply the matrix to the light. Then calculate the position on the shadow map. Check if matrix in the shadow map is the same as the one just used. If not repeat this step, until the matrices match.
After the right position on the shadow map has been found the shadow mapping algorithm continues as usual. This approach requires many texture lookups. The actual number depends on the possible view matrices. This value needs to be small to be a viable.
Testing needs to be done in camera space.
\subsubsection{Fake shadows}
The shadow mapping approach seems very expensive. Instead of shadow mapping a fake shadow can be used. It will not look realistic, but can give a sense of depth. For each lightsource a ray is cast from the character a cone is defined. The peak is a the characters origin. Its circlular surface has the light direction as normal. Every objects that can be shadowed checks if its fragments is inside the cone, if yes then it is shadowed. If the cone instersects with a portals, there needs to be another cone.

Instead of a cone something similar to shadow volumes can be used. some 2 shape infinitely far away. Calc distance, the see if inside the shape, the farther away the bigger the shape for testing.

\subsubsection{Make Portals Glow}
Make the portals glow. That way everything near them is lit. Shadows and lighting do not matter as they would not be visible anyways due to the portal glow dominating the lighting calculation. No seams can be seen near portals, and regular shadow mapping can be used.


\subsection{lighting}
Similar to shadows, lighting was also no really implemented. Only one directional light, which does not handle portals. Without correct handling spot and point lights could create visible seams at portal locations, just as shadows. Directional light would not produce seams, as the light direction will be the same no matter where an object is located.
Calculating the direction by subtracting the  Light also can travel through portals. Every light is used multiple times. Each of those has different teleport matrices applied and the usual light test is performed. Probably useful to do \ref{section:noveiw}, so this the same matrices as for the view can be used.
